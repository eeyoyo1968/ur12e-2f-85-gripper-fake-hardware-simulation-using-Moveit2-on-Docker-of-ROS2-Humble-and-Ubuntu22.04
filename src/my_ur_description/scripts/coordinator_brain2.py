...
import collections
import numpy as np
import threading  # Required to prevent the RCLError crash
...
class PickAndPlaceBrain(UR12eController):
    ...
    def class_callback(self, msg):
        # Update current_class so the bin logic always has the latest label
    ...
    def detection_callback(self, msg):
        ...
        if all(spread < self.stability_threshold):
            ...
            # Start the movement in a NEW THREAD to prevent RCLError
            self.is_busy = True
    ...
    def run_pick_place_sequence(self, pose):
        try:
            ...
            # Bin Sorting Logic based on current_class
            ...
            # 4. RESET FOR NEXT OBJECT
            self.get_logger().info("Resetting for next object...")
            self.pose_buffer.clear()
            self.target_class = "unknown"
            time.sleep(2.0) # Wait for camera to see empty table
            self.is_busy = False

def main(args=None):
    rclpy.init(args=args)
    brain = PickAndPlaceBrain()
    # Using SingleThreadedExecutor is now safe because motion is threaded
