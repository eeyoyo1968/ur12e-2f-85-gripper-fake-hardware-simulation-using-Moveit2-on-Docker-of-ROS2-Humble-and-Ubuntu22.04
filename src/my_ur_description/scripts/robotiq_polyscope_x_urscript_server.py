...
Uses Robotiq URCapX URScript functions (rq_activate, rq_move_and_wait)
on port 30002. This is simpler than Modbus and works if the Robotiq
URCapX is properly installed.
...
class RobotiqPolyscopeXURScriptServer(Node):
    def __init__(self):
        ...
        self.slave_id    = self.declare_parameter('slave_id', 0).value  # Default to 0 for PolyScope X
        ...
        self.get_logger().info("Uses Robotiq URCapX URScript functions (port 30002)")
    ...
    textmsg("ROS2: Setting gripper Slave ID to {self.slave_id}...")
...
"""
    ...
    def _send_gripper_urscript(self, position_rad: float) -> bool:
        """Send gripper move via Robotiq URScript functions"""
