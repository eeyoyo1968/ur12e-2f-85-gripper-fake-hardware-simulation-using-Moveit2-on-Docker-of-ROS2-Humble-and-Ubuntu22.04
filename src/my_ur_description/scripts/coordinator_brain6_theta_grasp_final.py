...
class PickAndPlaceBrain(UR12eController):
    ...
    def set_gripper(self, position):
        """Corrected 3-register mapping: [Action, Position, Speed/Force]"""
        try:
    ...
    def check_stability(self):
        if self.latest_pose is None or (self.latest_pose.position.x == 0.0 and self.latest_pose.position.y == 0.0):
            return False
    ...
    def execute_move(self):
        ...
        try:
            ...
            time.sleep(0.5)
            self.set_gripper(228) # Close (Verified limit)
            time.sleep(1.2) 
...
def main(args=None):
    ...
    time.sleep(4.0) # Wait for gripper calibration
