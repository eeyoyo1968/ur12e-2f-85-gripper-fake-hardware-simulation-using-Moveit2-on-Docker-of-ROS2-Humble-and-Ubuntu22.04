...
import collections
import numpy as np
import threading  # This is the key fix
...
class PickAndPlaceBrain(UR12eController):
    def __init__(self):
        ...
        # Subscribers
        self.obj_pose_sub = self.create_subscription(PoseStamped, '/grasp/pose', self.detection_callback, 10)
    ...
    def detection_callback(self, msg):
        ...
        if all(spread < self.stability_threshold):
            ...
            # START THREADED SEQUENCE
            self.is_busy = True
            # We launch the movement in a background thread so the executor doesn't crash
            move_thread = threading.Thread(target=self.run_pick_place_sequence, args=(msg.pose,))
            move_thread.start()
    ...
    def run_pick_place_sequence(self, pose):
        try:
            ...
            # 3. Bin Phase (Using current_class)
            ...
            # 5. Reset for next object
            self.get_logger().info("Resetting for next object...")
            self.pose_buffer.clear()
            self.target_class = "unknown"
            time.sleep(3.0) # Wait for camera to clear ghost images
            self.is_busy = False
...
def main(args=None):
    ...
    # Using SingleThreadedExecutor is now stable because motion is in a separate thread
